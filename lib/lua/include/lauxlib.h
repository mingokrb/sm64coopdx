...
struct ConstexprLibrary {
    ...
    ConstexprLibrary(const char *name, const luaL_Reg *funcs) : name(name), funcs(funcs) {
    }
};
...
#define PLUTO_NEWLIB(name)                                                                             \
    LUAMOD_API int luaopen_##name(lua_State *L) {                                                      \
        luaL_newlib(L, funcs_##name);                                                                  \
        return 1;                                                                                      \
    }                                                                                                  \
    const Pluto::PreloadedLibrary Pluto::preloaded_##name{ #name, funcs_##name, &luaopen_##name };
...
LUALIB_API FILE *(luaL_fopen) (const char *filename, size_t filename_len, const char *mode,
                               size_t mode_len);
...
#define pluto_newclassinst(L, T, ...)                                                                  \
    (T *) pluto_setupgcmt(L, new (lua_newuserdata(L, sizeof(T))) T(__VA_ARGS__), #T,                   \
                          [](lua_State *L2) {                                                          \
